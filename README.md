...
Q：为什么需要 Facotry 合约？（ Facotry 合约的作用是什么？）
A：1.工厂合约作为交易记录的注册表：每个新部署的交易合约都会在工厂中注册。这是一个重要的机制：通过查询注册表可以找到任何交易。通过拥有这样的注册表，当用户尝试将代币与另一个代币（而不是以太币）进行交换时，交易可以找到其他交易。2.Factory合约提供的另一个实用工具是能够在不涉及代码、节点、部署脚本和任何其他开发工具的情况下部署交易所。Factory实现了一个函数，允许用户通过调用此函数来创建和部署交易所。

Q：如何实现 Factory 合约？
A：1.工厂是一个注册表，因此我们需要一个数据结构来存储交换，这将是一个地址到地址的映射 - 它将允许通过它们的代币找到交换（uinswap-v1 中，一个 Exchange 只能 swap 一种 token）。2.接下来是 createExchange 功能，它允许通过简单地获取代币地址来创建和部署交易所。3.为了完成合约，我们还需要实现一个功能 - getExchange ，它将允许我们通过另一个合同的接口查询注册表。

Q：为什么在 Factory 合约中实现 getExchange 函数，而不是直接通过 tokenToExchange 调用？
A：我们正在使用 IFactory ，它是 Factory 合约的一个接口。在与其他合约（或面向对象编程中的类）进行交互时，使用接口是一个好的实践。然而，接口不允许访问状态变量，这就是为什么我们在Factory合约中实现了 getExchange 函数的原因 - 这样我们就可以通过接口使用合约。